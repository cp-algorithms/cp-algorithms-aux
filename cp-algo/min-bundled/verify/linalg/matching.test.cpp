#line 1 "verify/linalg/matching.test.cpp"
#define PROBLEM "https:#include <bits/stdc++.h>
...
#line 7 "verify/linalg/matching.test.cpp"
using namespace std;
...
for(int i=0;i<ssize(pivots);i++){for(int j=0;j<ssize(pivots);j++){B[i][j]=T[pivots[i]][pivots[j]];}}
B=B.inv().second;
vector<pair<int,int>>ans;
for(size_t i=0;i<size(pivots);i++){for(size_t j=0;j<size(pivots);j++){if(T[pivots[i]][pivots[j]]!=0&&B[i][j]!=0){ans.emplace_back(pivots[i],pivots[j]);
B.eliminate<gauss_mode::reverse>(i,j);
B.eliminate<gauss_mode::reverse>(j,i);
B.normalize();
for(int k=0;k<ssize(pivots);k++){B[i][k]=B[j][k]=0;}}}}
cout<<size(ans)<<"\n";
