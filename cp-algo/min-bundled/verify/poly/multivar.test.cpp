#line 1 "verify/poly/multivar.test.cpp"
...
namespace cp_algo::math::fft{template<modint_type base>
struct multivar{std::vector<base,cp_algo::big_alloc<base>>data;
std::vector<size_t,cp_algo::big_alloc<size_t>>ranks;
std::vector<size_t>dim;
size_t N;
size_t rank(size_t i){size_t cur=1,res=0,K=size(dim);
for(auto ni:dim){cur*=ni;
res+=i/cur;}
return res%K;}
...
ranks.resize(N);
for(auto[i,x]:ranks|std::views::enumerate){x=rank(i);}
checkpoint("multivar init");}
void read(){for(auto&it:data){std::cin>>it;}
checkpoint("multivar read");}
void print(){for(auto&it:data){std::cout<<it<<" ";}
std::cout<<"\n";
checkpoint("multivar write");}
void mul(multivar<base>const&b){assert(dim==b.dim);
size_t K=size(dim);
if(K==0){data[0]*=b.data[0];
return;}
std::vector<dft<base>>A,B;
size_t M=std::max(flen,std::bit_ceil(2*N-1)/2);
for(size_t i=0;i<K;i++){A.emplace_back(data|std::views::enumerate|std::views::transform(
[&](auto jx){auto[j,x]=jx;
...
for(size_t i=0;i<K;i++){dft<base>C(M);
cvector X=C.A;
for(size_t j=0;j<K;j++){size_t tj=(i-j+K)%K;
...
for(size_t j=0;j<N;j++){if(i==ranks[j]){data[j]=res[j];}}
checkpoint("store");}}};}
#line 8 "verify/poly/multivar.test.cpp"
using namespace std;
