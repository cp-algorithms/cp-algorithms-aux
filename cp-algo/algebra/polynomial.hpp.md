...
data:
  ...
  bundledCode: "#line 1 \"cp-algo/algebra/polynomial.hpp\"\n\n\n#line 1 \"cp-algo/algebra/common.hpp\"\
    \n\n\n#include <chrono>\n#include <random>\nnamespace algebra {\n    const int\
    ...
    \         return x.b.inv();\n                    }\n                }\n      \
    \      }\n        }\n        \n        uint64_t r;\n        constexpr modular():\
    \ r(0) {}\n        constexpr modular(int64_t rr): r(rr % m) {r = min<uint64_t>(r,\
    ...
    \ n);\n            if(a == b) {\n                a = A * A;\n            } else\
    \ {\n                a = A * dft<m>(b, n);\n            }\n        }\n    }\n\
    }\n\n#line 8 \"cp-algo/algebra/polynomial.hpp\"\nnamespace algebra {\n    template<typename\
    \ T>\n    struct poly {\n        std::vector<T> a;\n        \n        void normalize()\
    ...
    \  res.resize(max(n, res.size()));\n            return std::vector<T>(res.rbegin(),\
    \ res.rbegin() + n);\n        }\n        \n        poly reverse() const {\n  \
    ...
    \        }\n        };\n        \n        template<typename Q>\n        static\
    \ void concat(std::vector<Q> &a, std::vector<Q> const& b) {\n            for(auto\
    \ it: b) {\n                a.push_back(it);\n            }\n        }\n     \
    \   \n        // finds a transform that changes A/B to A'/B' such that\n     \
    \   // deg B' is at least 2 times less than deg A\n        static pair<std::vector<poly>,\
    \ transform> half_gcd(poly A, poly B) {\n            assert(A.deg() >= B.deg());\n\
    \            int m = (A.deg() + 1) / 2;\n            if(B.deg() < m) {\n     \
    ...
    \ {ai});\n            concat(ar, as);\n            return {ar, Tr * transform(ai)\
    \ * Ts};\n        }\n        \n        // return a transform that reduces A /\
    \ B to gcd(A, B) / 0\n        static pair<std::vector<poly>, transform> full_gcd(poly\
    \ A, poly B) {\n            std::vector<poly> ak;\n            std::vector<transform>\
    \ trs;\n            while(!B.is_zero()) {\n                if(2 * B.deg() > A.deg())\
    \ {\n                    auto [a, Tr] = half_gcd(A, B);\n                    concat(ak,\
    \ a);\n                    trs.push_back(Tr);\n                    tie(A, B) =\
    \ trs.back().adj().apply(A, B);\n                } else {\n                  \
    \  auto [a, R] = A.divmod(B);\n                    ak.push_back(a);\n        \
    \            trs.emplace_back(a);\n                    tie(A, B) = make_pair(B,\
    ...
    \        }\n                return ans.mod_xk(n);\n            }\n           \
    \ return nullopt;\n        }\n        \n        poly mulx(T a) const { // component-wise\
    \ multiplication with a^k\n            T cur = 1;\n            poly res(*this);\n\
    \            for(int i = 0; i <= deg(); i++) {\n                res.coef(i) *=\
    \ cur;\n                cur *= a;\n            }\n            return res;\n  \
    \      }\n\n        poly mulx_sq(T a) const { // component-wise multiplication\
    \ with a^{k choose 2}\n            T cur = 1, total = 1;\n            poly res(*this);\n\
    \            for(int i = 0; i <= deg(); i++) {\n                res.coef(i) *=\
    \ total;\n                cur *= a;\n                total *= cur;\n         \
    \   }\n            return res;\n        }\n\n        // be mindful of maxn, as\
    \ the function\n        // requires multiplying polynomials of size deg() and\
    \ n+deg()!\n        poly chirpz(T z, int n) const { // P(1), P(z), P(z^2), ...,\
    \ P(z^(n-1))\n            if(is_zero()) {\n                return std::vector<T>(n,\
    \ 0);\n            }\n            if(z == T(0)) {\n                std::vector<T>\
    \ ans(n, (*this)[0]);\n                if(n > 0) {\n                    ans[0]\
    \ = accumulate(begin(a), end(a), T(0));\n                }\n                return\
    \ ans;\n            }\n            auto A = mulx_sq(z.inv());\n            auto\
    \ B = ones(n+deg()).mulx_sq(z);\n            return semicorr(B, A).mod_xk(n).mulx_sq(z.inv());\n\
    \        }\n\n        // res[i] = prod_{1 <= j <= i} 1/(1 - z^j)\n        static\
    \ auto _1mzk_prod_inv(T z, int n) {\n            std::vector<T> res(n, 1), zk(n);\n\
    \            zk[0] = 1;\n            for(int i = 1; i < n; i++) {\n          \
    ...
    \    function<poly(poly const&, int, int)> compose_dac = [&getpow, &compose_dac](poly\
    \ const& f, int m, int N) {\n                if(f.deg() <= 0) {\n            \
    \        return f;\n                }\n                int k = m / 2;\n      \
    ...
    \n"
  code: "#ifndef ALGEBRA_POLYNOMIAL_HPP\n#define ALGEBRA_POLYNOMIAL_HPP\n#include\
    \ \"common.hpp\"\n#include \"modular.hpp\"\n#include \"fft.hpp\"\n#include <vector>\n\
    #include <optional>\nnamespace algebra {\n    template<typename T>\n    struct\
    \ poly {\n        std::vector<T> a;\n        \n        void normalize() { // get\
    \ rid of leading zeroes\n            while(!a.empty() && a.back() == T(0)) {\n\
    \                a.pop_back();\n            }\n        }\n        \n        poly(){}\n\
    \        poly(T a0) : a{a0}{normalize();}\n        poly(const std::vector<T> &t)\
    \ : a(t){normalize();}\n        \n        poly operator -() const {\n        \
    \    auto t = *this;\n            for(auto &it: t.a) {\n                it = -it;\n\
    \            }\n            return t;\n        }\n        \n        poly operator\
    \ += (const poly &t) {\n            a.resize(max(a.size(), t.a.size()));\n   \
    ...
    \ // drop first k coefficients\n            return std::vector<T>(begin(a) + min(k,\
...
layout: document
redirect_from:
- /library/cp-algo/algebra/polynomial.hpp
- /library/cp-algo/algebra/polynomial.hpp.html
