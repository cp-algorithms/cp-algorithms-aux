#line 1 "verify/poly/prod_sequence.test.cpp"
...
#line 5 "verify/poly/prod_sequence.test.cpp"
#include <bits/stdc++.h>
using namespace std;
...
multiset<polyn,decltype([](polyn const&a,polyn const&b){return a.deg()<b.deg();})>que={polyn(1)};
int D=0;
...
que.insert(polyn(a));}
while(que.size()>1){auto A=*begin(que);
que.erase(begin(que));
auto B=*begin(que);
que.erase(begin(que));
que.insert(A*B);}
