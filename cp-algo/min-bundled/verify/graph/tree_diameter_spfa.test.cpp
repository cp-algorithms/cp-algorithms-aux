#line 1 "verify/graph/tree_diameter_spfa.test.cpp"
...
#line 7 "verify/graph/tree_diameter_spfa.test.cpp"
#include <bits/stdc++.h>
using namespace std;
using namespace cp_algo::graph;
template<weighted_undirected_graph_type graph>
std::tuple<int64_t,node_index,std::vector<edge_index>>tree_diameter(graph const&g){auto[d1,_]=spfa(g,0);
node_index s=0;
for(auto v:g.nodes()){if(d1[v]>d1[s]){s=v;}}
auto[d2,pre]=spfa(g,s);
node_index t=0;
for(auto v:g.nodes()){if(d2[v]>d2[t]){t=v;}}
