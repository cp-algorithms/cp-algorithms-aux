#line 1 "verify/poly/sum_exp_poly.test.cpp"
...
#line 6 "verify/poly/sum_exp_poly.test.cpp"
#include <bits/stdc++.h>
using namespace std;
...
polyn xm1k(size_t k){polyn::Vector ans(k+1);
for(size_t i=0;i<=k;i++){ans[i]=(k-i)&1?-binom<base>(k,i):binom<base>(k,i);}
return ans;}
void solve(){int r,d;
cin>>r>>d;
...
id[0]=bpow(base(0),d);
id[1]=1;
for(int i=2;i<=d;i++){if(lp[i]==0){id[i]=bpow(base(i),d);
for(int j=i;j<=d;j+=i){lp[j]=i;}}else{id[i]=id[lp[i]]*id[i/lp[i]];}}
for(int i=0;i<=d;i++){ans+=H[i]*id[i];}
cout<<ans<<endl;}
