#line 1 "verify/linalg/adj.test.cpp"
...
#line 7 "verify/linalg/adj.test.cpp"
...
for(int i:views::iota(0,n)){for(int j:views::iota(0,n)){cin>>A[i][j];}}
for(int i:views::iota(0,n)){A[i][n]=cp_algo::random::rng();
A[n][i]=cp_algo::random::rng();}
auto[D,Ai]=A.inv();
for(int i:views::iota(0,n)){for(int j:views::iota(0,n)){if(D!=0){auto res=Ai[n][n]*Ai[i][j]-Ai[i][n]*Ai[n][j];
cout<<res*D<<" \n"[j+1==n];}else{cout<<0<<" \n"[j+1==n];}}}}
