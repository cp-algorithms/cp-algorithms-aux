...
data:
  ...
  bundledCode: "#line 1 \"cp-algo/algebra/poly.hpp\"\n\n\n#line 1 \"cp-algo/algebra/poly/impl/base.hpp\"\
    ...
    \        if(a == b) {\n            a = A * A;\n        } else {\n            a\
    \ = A * dft<m>(b, n);\n        }\n    }\n}\n\n#line 10 \"cp-algo/algebra/poly.hpp\"\
    \n#include <utility>\n#line 12 \"cp-algo/algebra/poly.hpp\"\nnamespace cp_algo::algebra\
    \ {\n    template<typename T>\n    struct poly_t {\n        std::vector<T> a;\n\
    \        \n        void normalize() {poly::impl::normalize(*this);}\n        \n\
    \        poly_t(){}\n        poly_t(T a0) : a{a0}{normalize();}\n        poly_t(const\
    \ std::vector<T> &t) : a(t){normalize();}\n        \n        poly_t operator -()\
    \ const {return poly::impl::neg(*this);}\n        \n        poly_t operator +=\
    ...
    \ i < t.a.size(); i++) {\n                a[i] -= t.a[i];\n            }\n   \
    \         normalize();\n            return *this;\n        }\n        poly_t operator\
    \ + (const poly_t &t) const {return poly_t(*this) += t;}\n        poly_t operator\
    \ - (const poly_t &t) const {return poly_t(*this) -= t;}\n        \n        poly_t\
    \ mod_xk(size_t k) const { // get first k coefficients\n            return std::vector<T>(begin(a),\
    \ begin(a) + std::min(k, a.size()));\n        }\n        \n        poly_t mul_xk(size_t\
    \ k) const { // multiply by x^k\n            auto res = a;\n            res.insert(begin(res),\
    \ k, 0);\n            return res;\n        }\n        \n        poly_t div_xk(size_t\
    \ k) const { // drop first k coefficients\n            return std::vector<T>(begin(a)\
    \ + std::min(k, a.size()), end(a));\n        }\n        \n        poly_t substr(size_t\
    \ l, size_t r) const { // return mod_xk(r).div_xk(l)\n            return std::vector<T>(\n\
    \                begin(a) + std::min(l, a.size()),\n                begin(a) +\
    \ std::min(r, a.size())\n            );\n        }\n        \n        poly_t operator\
    \ *= (const poly_t &t) {fft::mul(a, t.a); normalize(); return *this;}\n      \
    ...
    \ + 1);\n        }\n        \n        std::pair<poly_t, poly_t> divmod_slow(const\
    \ poly_t &b) const { // when divisor or quotient is small\n            std::vector<T>\
    ...
    \ end(res));\n            return {res, A};\n        }\n        \n        std::pair<poly_t,\
    \ poly_t> divmod_hint(poly_t const& b, poly_t const& binv) const { // when inverse\
    \ is known\n            assert(!b.is_zero());\n            if(deg() < b.deg())\
    \ {\n                return {poly_t{0}, *this};\n            }\n            int\
    \ d = deg() - b.deg();\n            if(std::min(d, b.deg()) < magic) {\n     \
    \           return divmod_slow(b);\n            }\n            poly_t D = (reverse().mod_xk(d\
    \ + 1) * binv.mod_xk(d + 1)).mod_xk(d + 1).reverse(d + 1);\n            return\
    \ {D, *this - D * b};\n        }\n        \n        std::pair<poly_t, poly_t>\
    \ divmod(const poly_t &b) const { // returns quotiend and remainder of a mod b\n\
    \            assert(!b.is_zero());\n            if(deg() < b.deg()) {\n      \
    \          return {poly_t{0}, *this};\n            }\n            int d = deg()\
    \ - b.deg();\n            if(std::min(d, b.deg()) < magic) {\n               \
    \ return divmod_slow(b);\n            }\n            poly_t D = (reverse().mod_xk(d\
    \ + 1) * b.reverse().inv(d + 1)).mod_xk(d + 1).reverse(d + 1);\n            return\
    \ {D, *this - D * b};\n        }\n        \n        template<typename Q>\n   \
    \     static void concat(std::vector<Q> &a, std::vector<Q> const& b) {\n     \
    \       for(auto it: b) {\n                a.push_back(it);\n            }\n \
    \       }\n        \n        // finds a linfrac<poly_t> that changes A/B to A'/B'\
    \ such that\n        // deg B' is at least 2 times less than deg A\n        static\
    \ std::pair<std::vector<poly_t>, linfrac<poly_t>> half_gcd(poly_t A, poly_t B)\
    \ {\n            assert(A.deg() >= B.deg());\n            int m = (A.deg() + 1)\
    \ / 2;\n            if(B.deg() < m) {\n                return {{}, {T(1), T(0),\
    \ T(0), T(1)}};\n            }\n            auto [ar, Tr] = half_gcd(A.div_xk(m),\
    ...
    \     static std::pair<std::vector<poly_t>, linfrac<poly_t>> full_gcd(poly_t A,\
    \ poly_t B) {\n            std::vector<poly_t> ak;\n            std::vector<linfrac<poly_t>>\
    \ trs;\n            while(!B.is_zero()) {\n                if(2 * B.deg() > A.deg())\
    ...
    \   // of the minimum linear recurrence for the sequence\n        poly_t min_rec_slow(int\
    \ d) const {\n            auto R1 = mod_xk(d + 1).reverse(d + 1), R2 = xk(d +\
    ...
    \    if(R2.deg() < Q2.deg()) {\n                    return Q2 / Q2.lead();\n \
    \               }\n            }\n            assert(0);\n        }\n        \n\
    \        static linfrac<poly_t> convergent(auto L, auto R) { // computes product\
    ...
    \ 1 < a.size(); i++) {\n                dr -= a[i + 1].deg();\n              \
    \  dp += a[i].deg();\n                if(dr < dp) {\n                    auto\
    \ ans = convergent(begin(a), begin(a) + i + 1);\n                    return ans.a\
    ...
    \ T &x) const {return poly_t(*this) *= x;}\n        poly_t operator / (const T\
    \ &x) const {return poly_t(*this) /= x;}\n        \n        poly_t conj() const\
    \ { // A(x) -> A(-x)\n            auto res = *this;\n            for(int i = 1;\
    \ i <= deg(); i += 2) {\n                res.a[i] = -res[i];\n            }\n\
    ...
    \            return a[idx];\n        }\n        \n        bool operator == (const\
    \ poly_t &t) const {return a == t.a;}\n        bool operator != (const poly_t\
    \ &t) const {return a != t.a;}\n        \n        poly_t deriv(int k = 1) { //\
    ...
    \              return -1;\n            }\n            int res = 0;\n         \
    \   while(a[res] == T(0)) {\n                res++;\n            }\n         \
    \   return res;\n        }\n        \n        poly_t log(size_t n) { // calculate\
    \ log p(x) mod x^n\n            assert(a[0] == T(1));\n            return (deriv().mod_xk(n)\
    \ * inv(n)).integr().mod_xk(n);\n        }\n        \n        poly_t exp(size_t\
    \ n) { // calculate exp p(x) mod x^n\n            if(is_zero()) {\n          \
    \      return T(1);\n            }\n            assert(a[0] == T(0));\n      \
    \      poly_t ans = T(1);\n            size_t a = 1;\n            while(a < n)\
    ...
    \ powmod_hint(int64_t k, poly_t const& md, poly_t const& mdinv) {\n          \
    \  if(k == 0) {\n                return poly_t(1);\n            } else {\n   \
    \             auto t = powmod_hint(k / 2, md, mdinv);\n                t = (t\
    \ * t).divmod_hint(md, mdinv).second;\n                if(k % 2) {\n         \
    \           t = (t * *this).divmod_hint(md, mdinv).second;\n                }\n\
    \                return t;\n            }\n        }\n\n        poly_t circular_closure(size_t\
    \ m) const {\n            if(deg() == -1) {\n                return *this;\n \
    \           }\n            auto t = *this;\n            for(size_t i = t.deg();\
    \ i >= m; i--) {\n                t.a[i - m] += t.a[i];\n            }\n     \
    \       t.a.resize(std::min(t.a.size(), m));\n            return t;\n        }\n\
    \n        static poly_t mul_circular(poly_t const& a, poly_t const& b, size_t\
    ...
    \ powmod_hint(k, md, mdinv);\n        }\n        \n        // O(d * n) with the\
    \ derivative trick from\n        // https://codeforces.com/blog/entry/73947?#comment-581173\n\
    ...
    \ poly_t(T(0)) : div_xk(i).pow(k, n - i * k).mul_xk(i * k);\n            }\n \
    ...
    \    }\n                return t;\n            }\n        }\n\n        poly_t\
    \ chirpz_inverse(T z, int n) const { // P(1), P(z), P(z^2), ..., P(z^(n-1))\n\
    \            if(is_zero()) {\n                return {};\n            }\n    \
    \        if(z == T(0)) {\n                if(n == 1) {\n                    return\
    \ *this;\n                } else {\n                    return std::vector{(*this)[1],\
    \ (*this)[0] - (*this)[1]};\n                }\n            }\n            std::vector<T>\
    \ y(n);\n            for(int i = 0; i < n; i++) {\n                y[i] = (*this)[i];\n\
    ...
    \              znk *= zn;\n            }\n\n            poly_t p_over_q = poly_t(y).chirpz(z,\
    \ n);\n            poly_t q = _1mzkx_prod(z, n);\n\n            return (p_over_q\
    \ * q).mod_xk(n).reverse(n);\n        }\n\n        static poly_t build(std::vector<poly_t>\
    ...
    \ R);\n            }\n        }\n\n        poly_t to_newton(std::vector<poly_t>\
    \ &tree, int v, auto l, auto r) {\n            if(r - l == 1) {\n            \
    \    return *this;\n            } else {\n                auto m = l + (r - l)\
    \ / 2;\n                auto A = (*this % tree[2 * v]).to_newton(tree, 2 * v,\
    \ l, m);\n                auto B = (*this / tree[2 * v]).to_newton(tree, 2 * v\
    ...
    \ }\n\n        std::vector<T> eval(std::vector<poly_t> &tree, int v, auto l, auto\
    ...
    \ end(x));\n        }\n        \n        poly_t inter(std::vector<poly_t> &tree,\
    ...
    \ auto resultant(poly_t a, poly_t b) { // computes resultant of a and b\n    \
    \        if(b.is_zero()) {\n                return 0;\n            } else if(b.deg()\
    ...
    \        }\n        \n        static poly_t ones(size_t n) { // P(x) = 1 + x +\
    \ ... + x^{n-1} \n            return std::vector<T>(n, 1);\n        }\n      \
    \  \n        static poly_t expx(size_t n) { // P(x) = e^x (mod x^n)\n        \
    \    return ones(n).borel();\n        }\n\n        static poly_t log1px(size_t\
    \ n) { // P(x) = log(1+x) (mod x^n)\n            std::vector<T> coeffs(n, 0);\n\
    \            for(size_t i = 1; i < n; i++) {\n                coeffs[i] = (i &\
    \ 1 ? T(i).inv() : -T(i).inv());\n            }\n            return coeffs;\n\
    \        }\n\n        static poly_t log1mx(size_t n) { // P(x) = log(1-x) (mod\
    \ x^n)\n            return -ones(n).integr();\n        }\n        \n        //\
    \ [x^k] (a corr b) = sum_{i} a{(k-m)+i}*bi\n        static poly_t corr(poly_t\
    \ a, poly_t b) { // cross-correlation\n            return a * b.reverse();\n \
    \       }\n\n        // [x^k] (a semicorr b) = sum_i a{i+k} * b{i}\n        static\
    \ poly_t semicorr(poly_t a, poly_t b) {\n            return corr(a, b).div_xk(b.deg());\n\
    ...
    \ / 2 + 1);\n            for(int i = 0; i <= deg(); i++) {\n                res[i\
    \ % 2].push_back(a[i]);\n            }\n            return {res[0], res[1]};\n\
    \        }\n        \n        // Find [x^k] P / Q\n        static T kth_rec(poly_t\
    \ P, poly_t Q, int64_t k) {\n            while(k > Q.deg()) {\n              \
    ...
    \ + 1))[k];\n        }\n        \n        poly_t inv(int n) const { // get inverse\
    \ series mod x^n\n            auto Q = mod_xk(n);\n            if(n == 1) {\n\
    \                return Q[0].inv();\n            }\n            // Q(-x) = P0(x^2)\
    ...
    \                Bqk = (Bqk * Bq).mod_xk(n);\n            }\n            return\
    \ ans;\n        }\n        \n        // compute A(B(x)) mod x^n in O(sqrt(pqn\
    \ log^3 n))\n        // preferrable when p = deg A and q = deg B\n        // are\
    \ much less than n\n        static poly_t compose_large(poly_t A, poly_t B, int\
    \ n) {\n            if(B[0] != T(0)) {\n                return compose_large(A.shift(B[0]),\
    \ B - B[0], n);\n            }\n            \n            int q = std::sqrt(n);\n\
    \            auto [B0, B1] = std::make_pair(B.mod_xk(q), B.div_xk(q));\n     \
    \       \n            B0 = B0.div_xk(1);\n            std::vector<poly_t> pw(A.deg()\
    \ + 1);\n            auto getpow = [&](int k) {\n                return pw[k].is_zero()\
    \ ? pw[k] = B0.pow(k, n - k) : pw[k];\n            };\n            \n        \
    \    std::function<poly_t(poly_t const&, int, int)> compose_dac = [&getpow, &compose_dac](poly_t\
    \ const& f, int m, int N) {\n                if(f.deg() <= 0) {\n            \
    \        return f;\n                }\n                int k = m / 2;\n      \
    \          auto [f0, f1] = std::make_pair(f.mod_xk(k), f.div_xk(k));\n       \
    \         auto [A, B] = std::make_pair(compose_dac(f0, k, N), compose_dac(f1,\
    \ m - k, N - k));\n                return (A + (B.mod_xk(N - k) * getpow(k).mod_xk(N\
    \ - k)).mul_xk(k)).mod_xk(N);\n            };\n            \n            int r\
    \ = n / q;\n            auto Ar = A.deriv(r);\n            auto AB0 = compose_dac(Ar,\
    \ Ar.deg() + 1, n);\n            \n            auto Bd = B0.mul_xk(1).deriv();\n\
    \            \n            poly_t ans = T(0);\n            \n            std::vector<poly_t>\
    \ B1p(r + 1);\n            B1p[0] = poly_t(T(1));\n            for(int i = 1;\
    \ i <= r; i++) {\n                B1p[i] = (B1p[i - 1] * B1.mod_xk(n - i * q)).mod_xk(n\
    \ - i * q);\n            }\n            while(r >= 0) {\n                ans +=\
    \ (AB0.mod_xk(n - r * q) * rfact<T>(r) * B1p[r]).mul_xk(r * q).mod_xk(n);\n  \
    \              r--;\n                if(r >= 0) {\n                    AB0 = ((AB0\
    \ * Bd).integr() + A[r] * fact<T>(r)).mod_xk(n);\n                }\n        \
    \    }\n            \n            return ans;\n        }\n    };\n    \n    static\
    \ auto operator * (const auto& a, const poly_t<auto>& b) {\n        return b *\
    \ a;\n    }\n};\n\n"
  code: "#ifndef CP_ALGO_ALGEBRA_POLYNOMIAL_HPP\n#define CP_ALGO_ALGEBRA_POLYNOMIAL_HPP\n\
    #include \"poly/impl/base.hpp\"\n#include \"common.hpp\"\n#include \"fft.hpp\"\
    \n#include <functional>\n#include <algorithm>\n#include <iostream>\n#include <optional>\n\
    #include <utility>\n#include <vector>\nnamespace cp_algo::algebra {\n    template<typename\
    \ T>\n    struct poly_t {\n        std::vector<T> a;\n        \n        void normalize()\
    \ {poly::impl::normalize(*this);}\n        \n        poly_t(){}\n        poly_t(T\
    \ a0) : a{a0}{normalize();}\n        poly_t(const std::vector<T> &t) : a(t){normalize();}\n\
    \        \n        poly_t operator -() const {return poly::impl::neg(*this);}\n\
    \        \n        poly_t operator += (const poly_t &t) {\n            a.resize(std::max(a.size(),\
    \ t.a.size()));\n            for(size_t i = 0; i < t.a.size(); i++) {\n      \
    \          a[i] += t.a[i];\n            }\n            normalize();\n        \
    ...
    \ i = 0; i < t.a.size(); i++) {\n                a[i] -= t.a[i];\n           \
    \ }\n            normalize();\n            return *this;\n        }\n        poly_t\
    \ operator + (const poly_t &t) const {return poly_t(*this) += t;}\n        poly_t\
    \ operator - (const poly_t &t) const {return poly_t(*this) -= t;}\n        \n\
...
documentation_of: cp-algo/algebra/poly.hpp
layout: document
redirect_from:
- /library/cp-algo/algebra/poly.hpp
- /library/cp-algo/algebra/poly.hpp.html
