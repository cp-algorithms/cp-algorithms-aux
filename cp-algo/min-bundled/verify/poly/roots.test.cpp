...
#define PROBLEM "https:#line 1 "cp-algo/math/poly.hpp"
...
#line 4 "verify/poly/roots.test.cpp"
#include <bits/stdc++.h>
using namespace std;
...
void find_roots_impl(polyn const&p,polyn::Vector&res){if(p.deg()==1){res.push_back(-p[0]/p[1]);}else if(p.deg()>1){auto A=polyn::gcd(polyn(polyn::Vector{(base)rng(),1}).powmod((mod-1)/2,p)-base(1),polyn(p));
find_roots_impl(A,res);
find_roots_impl(p/A,res);}}
auto find_roots(polyn const&p){polyn::Vector res;
