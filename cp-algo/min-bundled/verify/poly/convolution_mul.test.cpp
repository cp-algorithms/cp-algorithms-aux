...
cin>>a0;
for(int i=1;i<=p-1;i++){cin>>a[lg[i]];
as+=a[lg[i]];}
cin>>b0;
for(int i=1;i<=p-1;i++){cin>>b[lg[i]];
bs+=b[lg[i]];}
cp_algo::checkpoint("read");
base c0=(a0+as)*(b0+bs)-as*bs;
cout<<c0<<" ";
cp_algo::math::fft::mul(a,b);
for(size_t i=p-1;i<size(a);i++){a[i-(p-1)]+=a[i];}
for(int i=1;i<=p-1;i++){cout<<a[lg[i]]<<" ";}
