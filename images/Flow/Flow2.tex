...
\noindent
    ...
    \begin{tikzpicture}[scale=1.0,transform shape,auto,swap]
        ...
        \path[edge] (A) -- node[weight,above] {0/5} (B);
        \path[edge] (D) -- node[weight] {0/3} (A);
        \path[edge] (s) -- node[weight,below left] {0/4} (D);
        \path[edge] (s) -- node[weight,above left] {0/7} (A);
        \path[edge] (C) -- node[weight] {0/5} (t);
        \path[edge] (B) -- node[weight,above right] {0/8} (t);
        \path[edge] (A) -- node[weight,above right] {0/3} (C);
        \begin{scope}[transparency group, opacity=0.7]
            \path[selected edge] (s) -- node[weight,below] {\textbf 5} (A);
            \path[selected edge] (A) -- node[weight,below] {\textbf 5} (B);
            \path[selected edge] (B) -- node[weight,below] {\textbf 5} (t);
        \end{scope}
    \end{tikzpicture}
\end{document}
